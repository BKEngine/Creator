...
//user-defined
bool SCI_METHOD Document::SetSingleStyles(int start, int length, unsigned char styles)
{
	if (enteredStyling != 0) 
	{
		return false;
	}
	PLATFORM_ASSERT(start >= 0 && start + length <= Length());
	enteredStyling++;
	...
	{
		if (cb.SetStyleAt(start + i, styles))
		{
			if (!didChange) {
				startMod = endStyled;
			}
