...
protected:
	...
	public:
		...
		{
			_ASSERT_EXPR(node->next != NULL, W("end iterator不能再++"));
			this->node = node->next;
			return *this;
		}
		_Iterator& operator ++()
		...
		const _Iterator& operator --() const
		{
			_ASSERT_EXPR(node->last != NULL, W("begin iterator不能再--"));
			this->node = node->last;
			return *this;
		}
		_Iterator& operator --()
		...
		_Content *operator ->()
		{
			_ASSERT_EXPR(node->next != NULL, W("end没有->运算"));
			return &node->ct;
		}
...
public:
	...
	iterator find(const T &key) const
	{
		...
		while (node && node->index == h)
		{
			if (node->hashvalue == ha && node->ct.first == key)
			{
				it.node = node;
				return it;
			}
