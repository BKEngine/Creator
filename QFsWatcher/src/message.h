...
{
    friend class FileSystemPayload;
public:
    FileSystemEvent(const FileSystemEvent &r) noexcept;

    FileSystemEvent(FileSystemEvent &&original) noexcept;
...
private:
    FileSystemEvent(ChannelID channel_id,
...
public:
  ...
  const ChannelID &get_channel_id() const { return event.channel_id; }

  const FileSystemAction &get_filesystem_action() const { return event.action; }

  const EntryKind &get_entry_kind() const { return event.entry_kind; }

  const std::string &get_old_path() const { return event.old_path; }

  const std::string &get_path() const { return event.path; }

  const FileSystemEvent &get_event() const { return event; }
