...
FileSystemPayload::FileSystemPayload(ChannelID channel_id,
  ...
  string &&old_path,
  string &&path) :
  event(channel_id, action, entry_kind, move(old_path), move(path))
{
  //
}

FileSystemPayload::FileSystemPayload(FileSystemPayload &&original) noexcept :
    event(move(original.event))
{
  //
}
...
string FileSystemPayload::describe() const
{
  ...
  if (!event.old_path.empty()) {
    builder << " {" << event.old_path << " => " << event.path << "}";
  } else {
    builder << " " << event.path;
  }
  builder << "]";
...
FileSystemEvent::FileSystemEvent(FileSystemEvent &&original) noexcept :
...
FileSystemEvent::FileSystemEvent(const FileSystemEvent &original) noexcept :
    channel_id{original.channel_id},
    action{original.action},
    entry_kind{original.entry_kind},
    old_path{original.old_path},
    path{original.path}
{
